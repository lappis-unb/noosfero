class CustomFormsPluginProfileController < ProfileController
  ...
  def show
    ...
    if user
      @submission = CustomFormsPlugin::Submission.find_by form_id: @form.id, profile: user
    ...
    if request.post?
      begin
        raise 'Submission already present!' if user.present? && CustomFormsPlugin::Submission.find_by(form_id: @form.id, profile: user)
